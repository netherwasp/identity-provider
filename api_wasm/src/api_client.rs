use gloo_net::http::{Method, RequestBuilder};
use js_sys::Date;
use serde::Deserialize;
use serde_json::{Value, map::Values};
use serde_wasm_bindgen::from_value;
use std::cell::RefCell;
use std::time::Duration;
use wasm_bindgen::prelude::*;
use web_sys::RequestCredentials;

use crate::shared::interfaces::{CsrfJson, CsrfState};

thread_local! {
    static CSRF: RefCell<Option<CsrfState>> = RefCell::new(None);
}

const CSRF_TTL_MS: f64 = 50.0 * 60.0 * 1000.0;

pub fn set_csrf(token: String) {
    CSRF.with(|c| {
        *c.borrow_mut() = Some(CsrfState {
            token,
            fetched_at: Date::now(),
        });
    });
}

pub fn get_csrf() -> Option<String> {
    CSRF.with(|c| {
        c.borrow().as_ref().and_then(|state| {
            let age = Date::now() - state.fetched_at;
            if age < CSRF_TTL_MS {
                Some(state.token.clone())
            } else {
                None
            }
        })
    })
}

pub fn clear_csrf() {
    CSRF.with(|c| *c.borrow_mut() = None);
}

#[derive(Debug, Clone, Default)]
pub struct ApiClient {
    pub request_uri: String,
    pub headers: Vec<(String, String)>,
    pub token: Option<String>,
    pub cookie: bool,
}

impl ApiClient {
    pub fn new(base_uri: impl Into<String>) -> Self {
        Self {
            request_uri: base_uri.into(),
            ..Default::default()
        }
    }

    pub fn with_header(
        mut self,
        header_key: impl Into<String>,
        header_value: impl Into<String>,
    ) -> Self {
        self.headers.push((header_key.into(), header_value.into()));
        self
    }

    pub async fn with_csrf(self) -> Result<Self, JsValue> {
        if let Some(token) = get_csrf() {
            return Ok(self.with_header("X-CSRF-Token", token));
            // clear_csrf();
        }

        match self.clone().with_cookie().get_csrf().await {
            Ok((_, Some(token))) => {
                set_csrf(token.clone());
                Ok(self.with_header("X-CSRF-Token", token))
            }
            Ok((_, None)) => Err("No X-CSRF-Token in response header".into()),
            Err(e) => {
                clear_csrf();
                Err(e.into())
            }
        }
    }

    pub fn with_token(self, token: impl Into<String>) -> Self {
        self.with_header("Authorization", format!("Bearer {}", token.into()))
    }

    pub fn with_cookie(mut self) -> Self {
        self.cookie = true;
        self
    }

    pub fn request_builder(&self, method: Method, endpoint: &str) -> RequestBuilder {
        let req = RequestBuilder::new(&format!("{}{}", self.request_uri, endpoint)).method(method);

        // Apply all headers
        let req = self
            .headers
            .iter()
            .fold(req, |r, (key, value)| r.header(key, value));

        if self.cookie {
            req.credentials(RequestCredentials::Include)
        } else {
            req
        }
    }

    pub async fn get_csrf(&self) -> Result<(String, Option<String>), String> {
        let req = self.request_builder(Method::GET, "/csrf");
        let response = req
            .build()
            .map_err(|e| e.to_string())?
            .send()
            .await
            .map_err(|e| e.to_string())?;

        let csrf_token = response
            .headers()
            .get("X-CSRF-Token")
            .map(|v| v.to_string());

        let text = response.text().await.map_err(|e| e.to_string())?;
        Ok((text, csrf_token))
    }

    // API METHODS
    pub async fn request(
        &self,
        method: &str,
        endpoint: &str,
        body: Option<&str>,
    ) -> Result<String, String> {
        let body_value: Value = body
            .filter(|s| !s.is_empty())
            .and_then(|s| serde_json::from_str(s).ok())
            .unwrap_or(Value::Null);
        match method.to_uppercase().as_str() {
            "GET" => {
                let req = self.request_builder(Method::GET, endpoint);
                let query_params = if let Some(params) = body_value.as_object() {
                    params
                        .iter()
                        .map(|(k, v)| (k.as_str(), v.as_str().unwrap_or_default()))
                        .collect::<Vec<(&str, &str)>>()
                } else {
                    Vec::new()
                };
                let req = req.query(query_params).build().map_err(|e| e.to_string())?;

                req.send()
                    .await
                    .map_err(|e| e.to_string())?
                    .text()
                    .await
                    .map_err(|e| e.to_string())
            }
            "POST" => {
                let req = self.request_builder(Method::POST, endpoint);
                let req = req.json::<Value>(&body_value).map_err(|e| e.to_string())?;

                req.send()
                    .await
                    .map_err(|e| e.to_string())?
                    .text()
                    .await
                    .map_err(|e| e.to_string())
            }
            inv => Err(format!("{inv} METHOD NOT ALLOWED")),
        }
    }
}
